...
(
	FK_ID_KEYWORD BIGINT NOT NULL REFERENCES KEYWORDS(ID)
	,FK_ID_RECEPTA BIGINT NOT NULL REFERENCES RECEPTES(ID)
	,PRIMARY KEY (FK_ID_KEYWORD,FK_ID_RECEPTA)
);

-- Comentaris a les receptes
DROP TABLE IF EXISTS COMENTARIS CASCADE;
CREATE TABLE COMENTARIS
(
	ID_COMENTARI BIGINT AUTO_INCREMENT NOT NULL PRIMARY KEY
	,FK_ID_RECEPTA BIGINT NOT NULL REFERENCES RECEPTES(ID)
	,TS_CREACIO TIMESTAMP NOT NULL
	,AUTOR NVARCHAR(50) NOT NULL
	,COMENTARI NVARCHAR(2500) NOT NULL
...
SELECT R.*, K.NOM AS KEYWORD, KR.FK_ID_KEYWORD FROM VRECEPTES R
LEFT JOIN KEYWORDS_RECEPTES KR
ON R.ID=KR.FK_ID_RECEPTA
LEFT JOIN KEYWORDS K
ON KR.FK_ID_KEYWORD=K.ID
...
LEFT JOIN KEYWORDS K ON K.ID=KR.FK_ID_KEYWORD;
